		...
		public override FootstepEffect GetFootstepAt(Vector3 position, string effectTag)
		{
				...
				this.tilemap.Length > 0)
			{
				for(int i = 0; i < this.tilemap.Length; i++)
				{
					...
					if(sprite != null)
					{
						for(int j = 0; j < this.textureMaterials.Count; j++)
						{
							FootstepEffect effect = this.textureMaterials[j].GetEffect(sprite, effectTag);
							if(effect != null)
							{
								return effect;
							}
						...
						{
							FootstepEffect effect = FootstepManager.Instance.GetFootstepFor(sprite, effectTag);
							if(effect != null)
							{
								return effect;
							}

...
namespace GamingIsLove.Footsteps
{
    ...
    public class Footstepper : MonoBehaviour
    {
        ...
        // 추가
        public TerrainFootstepSource terrainFootstepSource = new();

        public Animator animator;

        public Texture2D SandDarkerImage;

        public Texture2D SandSwimImage;


        private bool onWater = false;
        private bool playSwim = false;
        ...
        {
            //FuncTest.Instance.SetLogUI($"<Footstepper/start>");
        ...
        protected virtual void Update()
        {
            ...
            if (this.speed.x > 0)
            {
                //FuncTest.Instance.SetLogUI($"[Footstepper/Update] 2", 0, autoPlayTimeout);
                this.autoPlayTimeout -= Time.deltaTime;

                if (this.autoPlayTimeout <= 0)
                {
                    //FuncTest.Instance.SetLogUI($"[Footstepper/Update] 3");
        ...
        public virtual void CalculateSpeed()
        {
            ...
            this.lastPosition = this.transform.position;
            //FuncTest.Instance.SetLogUI($"", speed.x);
        }
		...
		*/
        ...
        {
            //FuncTest.Instance.SetLogUI($"[Footstepper/PlayFootstep]");
        ...
        {
            //FuncTest.Instance.SetLogUI($"[Footstepper/FindFootstep]");
            ...
            if (result != null)
            {
                ...
                {
                    if (terrainFootstepSource.GetTextureAt(hitPosition) == SandDarkerImage && !onWater)
                    {
                        //FuncTest.Instance.SetLogUI($"[Footstepper/FindFootstep/footstepSource]");
                        //Debug.Log(this.name + "거북이가 물에 들어감");
                        animator.SetTrigger("Turtle_Ani_Swim_Try");
                        onWater = true;
                    }

                    else if (terrainFootstepSource.GetTextureAt(hitPosition) == SandSwimImage && !playSwim)
                    {
                        //Debug.Log(this.name + "거북이가 Swim 시작함");
                        animator.SetTrigger("Turtle_Ani_Swim");
                        playSwim = true;
                    }
		...
		*/
        ...
        public virtual void FootstepIndex(int index)
        {
                    ...
                    this.speed.x >= this.minSpeed))
            {
                ...
                //FuncTest.Instance.SetLogUI($"[Footstepper/FootstepIndex] 1");
            }
        }
